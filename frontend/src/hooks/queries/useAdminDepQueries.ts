...
export const adminDepKeys = {
  cantEstudiantes: (carrera_id: number) =>
    ["cantEstudiantes", carrera_id] as const,
  conteoPorRiesgo: (carrera_id: number) =>
    ["conteoPorRiesgo", carrera_id] as const,
  ...
  evolucionScore: (anio: number, carrera_id: number) =>
    ["evolucionScore", anio, carrera_id] as const,
  estudiantesPorCarrera: (carrera_id: number) =>
    ["estudiantesPorCarrera", carrera_id] as const,
  estudiantePorDni: (dni: string) => ["estudiantePorDni", dni] as const,
  estudiantePorAnio: (anio: number) => ["estudiantePorAnio", anio] as const,
  estudiantePorRiesgo: (riesgo: string) =>
    ["estudiantePorRiesgo", riesgo] as const,
  historialAlertas: (student_id: string) =>
    ["historialAlertas", student_id] as const,
  historialAlertasGenerales: (carrera_id: string) =>
    ["historialAlertasGenerales", carrera_id] as const,
};

export const useConteoEstudiantes = (carrera_id: number) => {
  return useQuery({
    queryKey: adminDepKeys.cantEstudiantes(carrera_id),
    queryFn: () => getConteoEstudiantes({ carrera_id }),
  });
};

export const useConteoPorRiesgo = (carrera_id: number) => {
  return useQuery({
    queryKey: adminDepKeys.conteoPorRiesgo(carrera_id),
    queryFn: () => getConteoPorRiesgo({ carrera_id }),
  });
};
