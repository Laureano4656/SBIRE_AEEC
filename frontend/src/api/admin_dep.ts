...
} from "../types/admin_dep";

const PREFIX = "/dashboard-admin-dep";

export const getConteoEstudiantes = async ({
  carrera_id,
}: {
  carrera_id: number;
}) => {
  const response = await axiosInstance.get<{ cantidad: number }>(
    `${PREFIX}/estadisticas/estudiantes`,
    { params: { carrera_id } },
  );
  return response.data;
};

export const getConteoPorRiesgo = async ({
  carrera_id,
}: {
  carrera_id: number;
}) => {
...
export const getTotalCriticos = async ({
  ...
  const response = await axiosInstance.get<{ total: number }>(
    `${PREFIX}/estadisticas/totales/criticos`,
    { params: { carrera_id } },
  );
  return response.data;
};
...
export const getTotalAlertasNuevas = async ({
  ...
  const response = await axiosInstance.get<{ total: number }>(
    `${PREFIX}/estadisticas/totales/alertas-nuevas`,
    { params: { carrera_id } },
  );
  return response.data;
};

export const getTotalIntervenciones = async ({
  carrera_id,
}: {
  carrera_id: number;
}) => {
  const response = await axiosInstance.get<{ total: number }>(
    `${PREFIX}/estadisticas/totales/intervenciones`,
    { params: { carrera_id } },
  );
  return response.data;
};
...
export const getHistorialAlertas = async (student_id: string) => {
  const response = await axiosInstance.get<EventoCronologicoResponse[]>(
    `${PREFIX}/estudiantes/${student_id}/historial`,
  );
  return response.data;
};
...
export const getEstudiantesPorCarrera = async (carrera_id: number) => {
  const response = await axiosInstance.get<EstudianteDashboardAdminResponse[]>(
    `${PREFIX}/estudiantes/carrera`,
    { params: { carrera_id } },
  );
  return response.data;
};
