...
class PesoCriteriosServices:
    ...
    async def resolver_matriz_ahp(
        self, criterios: List[int], comparaciones: List[Comparacion]
    ):
        n = len(criterios)
        if n == 1:
    ...
    async def calcular_ahp_jerarquico(
        ...
        if nodo_actual in jerarquia:
            ...
            pesos_locales, cr = await self.resolver_matriz_ahp(
                hijos, comparaciones_hijos
            )
            reportes_cr[nodo_actual] = cr

            for hijo in hijos:
                peso_global_hijo = pesos_locales[hijo] * peso_global_padre
                await self.calcular_ahp_jerarquico(
                    hijo,
                    jerarquia,
    ...
    async def obtener_listado_indicadores_preguntas(
        self, carrera_id: int
