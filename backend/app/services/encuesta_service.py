...
from app.schemas.encuesta import EncuestaHistoricoResponse, EstadisticaEventoResponse, EncuestaEstudianteDetalle, RespuestaDetalle, FormularioEncuestaResponse, BloqueAcademico, MateriaResponse, RespuestaItemSubmit, RespuestaPrevia, OpcionEncuestaResponse, PreguntaParaEncuesta
...
class EncuestaService:
    ...
    async def obtener_metricas_por_evento_y_carrera(self, carrera_id: int) -> list[EstadisticaEventoResponse]:
        """
    ...
    async def obtener_historico_respuestas_ultimo_anio(self, carrera_id: int, evento_id: int) -> list[EncuestaHistoricoResponse]:
        anio_actual = str(datetime.now().year)
        
        # 1. Traer asignaciones completadas
        asignaciones = await self.repo.get_asignaciones_completadas_detalles(carrera_id, evento_id, anio_actual)
        if not asignaciones:
            return []
        ...
        pregunta_ids = [p.id for p in preguntas_crudas]
        dicc_opciones = self._agrupar_opciones(await self.repo.get_opciones(pregunta_ids), pregunta_ids)
        ...
        for r in respuestas_bulk:
            dicc_respuestas_por_asignacion[r["asignacion_id"]][(r["pregunta_id"], r["materia_id"])] = RespuestaPrevia(**dict(r))
