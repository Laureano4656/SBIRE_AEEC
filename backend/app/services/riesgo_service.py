...
class RiesgoService:
    ...
    async def tarea_background_recalcular_por_configuracion(
        pool: asyncpg.Pool, carrera_id: int, etapa: str
    ):
        """
        ...
        try:
            ...
            print(
                f"Iniciando recálculo masivo para {len(alumnos)} estudiantes de la carrera {carrera_id}..."
            )
                ...
                try:
                    async with pool.acquire() as conn:
                        ...
                        await service.calcular_y_guardar_riesgo(
                            estudiante_id=alumno["estudiante_id"], etapa=etapa
                        )
                except Exception as e:
                    # Logueamos si falla uno, pero el loop sigue con el resto
                    print(
                        f"Error al recalcular alumno {alumno['estudiante_id']}: {str(e)}"
                    )

            print(
                f"Recálculo masivo de la carrera {carrera_id} finalizado exitosamente."
            )
        ...
        try:
            async with pool.acquire() as conn:
                ...
                if estudiante:
                    service = RiesgoService(conn)
                    await service.calcular_y_guardar_riesgo(
                        estudiante_id, estudiante["etapa"]
                    )

        except Exception as e:
            print(
                f"Error recalculando semáforo del estudiante {estudiante_id}: {str(e)}"
            )

    @staticmethod
    async def tarea_background_recalcular_masivo(
        pool: asyncpg.Pool, estudiante_ids: list[int]
    ):
        """
        Procesa una lista plana de IDs. Ideal para la importación masiva de notas.
        """
        print(
            f"Iniciando recálculo por actualización de notas para {len(estudiante_ids)} estudiantes..."
        )

        for est_id in set(
            estudiante_ids
        ):  # Usamos set() por si en el Excel un alumno sale 2 veces
            try:
                async with pool.acquire() as conn:
                    repo = RiesgoRepository(conn)
    ...
    # ==========================================
    def _normalizar_riesgo(
        self, valor_numerico: float, config: dict[str, Any]
    ) -> float:
    ...
    def _determinar_color(
        self, score: float, umbral_amarillo: float, umbral_rojo: float
    ) -> str:
    ...
    async def calcular_y_guardar_riesgo(self, estudiante_id: int, etapa: str) -> int:
        ...
        if not config_db:
            raise HTTPException(
                status_code=404,
                detail=f"No hay configuración de indicadores activa para la carrera {carrera_id}.",
            )
        ...
        print(f"Pesos AHP obtenidos para carrera {carrera_id}: {pesos_ahp}")
        # 4. Buscar la última "foto" del estudiante (sus asignaciones más recientes por evento)
        asignaciones_recientes = (
            await self.repo.obtener_ultimas_asignaciones_por_estudiante(estudiante_id)
        )
        ...
        print(
            f"Asignaciones recientes para estudiante {estudiante_id}: {asignacion_ids}"
        )
        ...
        for row in respuestas_crudas:
            ...
            if (
                row["tipo_pregunta"] == "opcion_multiple"
                and row["valor_riesgo_manual"] is not None
            ):
                riesgo_pregunta = float(row["valor_riesgo_manual"])
            elif (
                row["tipo_pregunta"] in ["numero", "escala"]
                and row["valor_numerico"] is not None
                and row["configuracion_riesgo"]
            ):
            ...
            detalles_insert.append(
                {
                    ...
                    "nivel": self._determinar_nivel_riesgo(
                        promedio_indicador, umbral_amarillo, umbral_rojo
                    ),
        ...
        suma_pesos_utilizados = sum(
            pesos_ahp.get(ind_id, 0.0) for ind_id in riesgos_por_indicador.keys()
        )
        ...
        estado_general = self._determinar_nivel_riesgo(
            score_total_acumulado, umbral_amarillo, umbral_rojo
        )
        ...
        if estado_general == "alto" or estado_general == "critico":
            ...
            peores_indicadores = [
                ...
                if (d["nivel"] == "alto" or d["nivel"] == "critico")
            ]
            ...
            for indicador in peores_indicadores:
                try:
                    ...
                    print(
                        f"Error al crear alerta para estudiante {estudiante_id}: {str(e)}"
                    )
    ...
    async def armar_semaforo_estudiante(self, estudiante_id: int) -> SemaforoResponse:
        ...
        config_db = await self.repo.obtener_configuracion_activa(
            estudiante["carrera_id"], estudiante["etapa"]
        )
        if not config_db:
            raise HTTPException(
                status_code=404,
                detail="Configuración de semáforo no encontrada para este estudiante.",
            )
        ...
        if not total_row:
            raise HTTPException(
                status_code=404, detail="El estudiante no tiene cálculos de riesgo aún."
            )
        ...
        estado_general = self._determinar_color(
            riesgo_global, umbral_amarillo, umbral_rojo
        )
    ...
    async def armar_semaforo_estudiantes(
        self, carrera_id: int
        ...
        for estudiante_id in estudiantes_id:
            try:
                semaforos.append(
                    await self.armar_semaforo_estudiante(estudiante_id["estudiante_id"])
                )

            except HTTPException as http_ex:
                if http_ex.status_code == 404:
                    print(
                        f"Saltando estudiante {estudiante_id}: aún no tiene cálculos de riesgo."
                    )
                    continue
                ...
                print(
                    f"Error inesperado al procesar estudiante {estudiante_id}: {str(e)}"
                )
                continue
        return semaforos
